/**
 * Terminal component - manages non-theatre terminal instances (accordion terminals)
 * Theatre mode functionality is now in ./theatre/
 */

import { Terminal } from '@xterm/xterm';
import { FitAddon } from '@xterm/addon-fit';
import { createIcons, Maximize2 } from 'lucide';
...
export async function createTerminal(
  ...
  createIcons({ icons: terminalIcons, nodes: [container] });
  ...
  try {
    ...
    instance.cleanupData = window.api.pty.onData(result.ptyId, (data) => {
      ...
      if (theatreState.projectPath === projectPath) {
        scheduleGitStatusRefresh();
      }
    });
